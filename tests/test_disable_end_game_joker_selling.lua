...
local function make_sell_button()
    local text_node = { config = { text = '$5' } }
    local button = {
        config = { button = 'sell_card' },
        children = { text_node },
        removed = false,
    }
...
local function new_card(cost, with_sell_ui, use_named_child)
    local card = {
        sell_cost = cost,
        config = {},
        children = {},
    ...
    function card.UIBox:recalculate()
        self.recalculated = true
    end

    if with_sell_ui then
        local sell_button = make_sell_button()
        if use_named_child then
            card.children.center = { children = { sell_button } }
        else
            card.children[1] = sell_button
        end
    end
...
local function test_mark_card_unsellable()
    ...
    assert_true(card.UIBox.recalculated == true, 'card UI should be recalculated when removing sell UI')
    assert_true(#card.children == 0, 'sell UI should be removed from card children')
end

local function test_disable_card_area_selling()
    local area = { config = {}, cards = { new_card(2, true), new_card(nil) } }
    ...
    for _, card in pairs(area.cards) do
        ...
        assert_true(card.config.can_sell == false, 'card config should disable selling')
        assert_true(card.UIBox.recalculated == true, 'card UI should be recalculated when removing sell UI')
        assert_true(#card.children == 0, 'sell UI should be removed from cards in disabled areas')
    end
...
local function test_remove_sell_affordance_handles_named_children()
    local card = new_card(3, true, true)
    MP.UTILS.mark_card_unsellable(card)
    local center_children = card.children.center and card.children.center.children or {}
    local has_children = false
    for _ in pairs(center_children or {}) do
        has_children = true
        break
    end
    assert_true(not has_children, 'named child containers should have sell UI removed')
end
...
local function test_card_area_emplace_hook()
    ...
    assert_true(#card.children == 0, 'sell UI should be removed for cards added after disabling')
end
...
local function run()
    ...
    test_remove_sell_affordance_handles_named_children()
